%rectangular plate, upper boundary and lower boundary are fully clamped, left boundary and right boundary are free, the central crack is parallel to the upper and lower boundaries. 3 segments are used.
clear all
clc
format long
symm=0; %symmetry indicator, 0: symmetric, 1: antisymmetric
n=24; %the number of terms of admissible functions in each of the two coordinate directions
n2=n^2;
n3=2*n2;
a=0.32; 
b=2*0.32; %a, b are the plate dimensions in x and y directions
h=0.002; %thickness
c=0.6*a; %crack length
a1=(a-c)/2;
a2=c;
a3=a1;
b1=b/2;
b2=b/2;
b3=b/2; %dimensions of each segment
al1=a1/b1;
al2=a2/b2;
al3=a3/b3; %aspect ratios of segments
nu=0.31; %the Poisson's ratio
...
if symm==0 %symmetric
	ks4=1e10; 
	kp4=0;
	kr4=1e10; %lower edge, slip shear condition
else %antisymmetric
	ks4=0;
	kp4=1e10; 
	...
	for i=1:n 
		...
		for nn=1:n
			for j=1:n
				...
				M(u+n3,v+n3)=Emi00(m,nn)*Fni00(i,j)*a3*b3*Rho*h;
			end
		end
	end
end
...
for m=1:n
	for i=1:n
		...
		for nn=1:n
			for j=1:n
				v=j+(nn-1)*n;
				%there are 3 plate segments, so imagine the dimension of the stiffness matrix K is 3X3
				%for K11
				...
				K(u,v)=K(u,v)+kct12*KP2(m)*KP2(nn)*Fni00(i,j)*b1; %part of the translational boundary condition between segment 1 and segment 2
				...
				K(u+n2,v+n2)=K(u+n2,v+n2)+kct12*KP1(m)*KP1(nn)*Fni00(i,j)*b1; %part of the translational boundary condition between segment 2 and segment 1
				...
				K(u+n2,v+n2)=K(u+n2,v+n2)+kct23*KP2(m)*KP2(nn)*Fni00(i,j)*b1; %part of the translational boundary condition between segment 2 and segment 3
				K(u+n2,v+n2)=K(u+n2,v+n2)+kcr23*KC2(m)/a2*KC2(nn)/a2*Fni00(i,j)*b1; %part of the rotational boundary condition between segment 2 and segment 3
				...
				%for K33
				K(u+n3,v+n3)=K(u+n3,v+n3)+(Emi22(m,nn)*Fni00(i,j)+al3^4*Emi00(m,nn)*Fni22(i,j)+nu*al3^2*Emi02(m,nn)*Fni20(i,j)+nu*al3^2*Emi20(m,nn)*Fni02(i,j)+2*(1-nu)*al3^2*Emi11(m,nn)*Fni11(i,j))*D*b1/a3^3; %the segment 3 itself
				K(u+n3,v+n3)=K(u+n3,v+n3)+kct23*KP1(m)*KP1(nn)*Fni00(i,j)*b1; %part of the translational boundary condition between segment 3 and segment 2
				K(u+n3,v+n3)=K(u+n3,v+n3)+kcr23*KC1(m)/a3*KC1(nn)/a3*Fni00(i,j)*b1; %part of the rotational boundary condition between segment 3 and segment 2
				K(u+n3,v+n3)=K(u+n3,v+n3)+kp2*KP4(i)*KP4(j)*Emi00(m,nn)*a3; %the translational boundary condition for the upper edge of segment 3
				K(u+n3,v+n3)=K(u+n3,v+n3)+kr2*KC4(i)/b1*KC4(j)/b1*Emi00(m,nn)*a3; %the rotational boundary condition for the upper edge of segment 3
				K(u+n3,v+n3)=K(u+n3,v+n3)+kp4*KP3(i)*KP3(j)*Emi00(m,nn)*a3; %the translational boundary condition for the lower edge of segment 3
				K(u+n3,v+n3)=K(u+n3,v+n3)+ks4*KS3(i)*KS3(j)*Emi00(m,nn)*a3; %the slip shear boundary condition for the lower edge of segment 3
				K(u+n3,v+n3)=K(u+n3,v+n3)+kr4*KC3(i)/b1*KC3(j)/b1*Emi00(m,nn)*a3; %the rotational boundary condition for the lower edge of segment 3
				K(u+n3,v+n3)=K(u+n3,v+n3)+kp3*KP2(m)*KP2(nn)*Emi00(i,j)*b1; %the translational boundary condition for the right edge of segment 2
				K(u+n3,v+n3)=K(u+n3,v+n3)+kr3*KC2(m)/a3*KC2(nn)/a3*Emi00(i,j)*b1; %the rotational boundary condition for the right edge of segment 2
				%for K12
				...
				%for K23
				K(u+n2,v+n3)=K(u+n2,v+n3)-kct23*KP2(m)*KP1(nn)*Fni00(i,j)*b1; %part of the translational boundary condition between segment 2 and segment 3
				K(u+n2,v+n3)=K(u+n2,v+n3)-kcr23*KC2(m)/a2*KC1(nn)/a3*Fni00(i,j)*b1; %part of the rotational boundary condition between segment 2 and segment 3
				%for K21
				...
				%for K32
				K(u+n3,v+n2)=K(u+n3,v+n2)-kct23*KP2(nn)*KP1(m)*Fni00(i,j)*b1; %part of the translational boundary condition between segment 2 and segment 3
				K(u+n3,v+n2)=K(u+n3,v+n2)-kcr23*KC2(nn)/a2*KC1(m)/a3*Fni00(i,j)*b1; %part of the rotational boundary condition between segment 2 and segment 3
			end
		end
	end
end
...
for MODE_NUMBER=1:3	
	for i=1:size(x_nodes,2)
		...
		elseif x>a1&&x<=a1+a2 %when the point is in the middle segment (segment 2)
			EIGENVECTOR=real(X(n2+1:n3,MODE_NUMBER))';
			for j=1:size(y_nodes,2)
				y=y_nodes(j);
				SET_X(1)=1;
				SET_X(2)=(x-a1)/a2;
		...
		else %when the point is in the right segment (segment 3)
			EIGENVECTOR=real(X(n3+1:3*n^2,MODE_NUMBER))';
			for j=1:size(y_nodes,2)
				y=y_nodes(j);
				SET_X(1)=1;
				SET_X(2)=(x-a1-a2)/a3;
				SET_X(3)=((x-a1-a2)/a3)^2;
				SET_Y(1)=1;
				SET_Y(2)=y/b1;
				SET_Y(3)=(y/b1)^2;
				for k=1:n-3
					SET_X(k+3)=cos(k*pi*(x-a1-a2)/a3);
					SET_Y(k+3)=cos(k*pi*y/b1);
				end
				counter=0;
				for p=1:n
	...
	if symm==0 %symmetric mode shape
		...
		figure
		surf(Y_nodes,X_nodes,MODE) %mode shape of full plate
		axis off
